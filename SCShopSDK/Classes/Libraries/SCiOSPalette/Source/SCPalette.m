...
#import "SCPalette.h"
#import "SCPaletteSwatch.h"
#import "SCPaletteColorUtils.h"
#import "SCPriorityBoxArray.h"
    ...
    switch (dimension) {
        ...
        case COMPONENT_GREEN:
            ...
            for (NSInteger i = lower; i <= upper; i++) {
                ...
                NSInteger newColor = [SCPaletteColorUtils quantizedGreen:color] << (QUANTIZE_WORD_WIDTH + QUANTIZE_WORD_WIDTH)
                | [SCPaletteColorUtils quantizedRed:color]  << QUANTIZE_WORD_WIDTH | [SCPaletteColorUtils quantizedBlue:color];
        ...
        case COMPONENT_BLUE:
            ...
            for (NSInteger i = lower; i <= upper; i++) {
                NSInteger color = [_distinctColors[i] intValue];
                NSInteger newColor =  [SCPaletteColorUtils quantizedBlue:color] << (QUANTIZE_WORD_WIDTH + QUANTIZE_WORD_WIDTH)
                | [SCPaletteColorUtils quantizedGreen:color]  << QUANTIZE_WORD_WIDTH
                | [SCPaletteColorUtils quantizedRed:color];
                _distinctColors[i] = [NSNumber numberWithInteger:newColor];
            }
            break;
    }
 ...
 */
- (SCPaletteSwatch*)getAverageColor{
    NSInteger redSum = 0;
    ...
    for (NSInteger i = _lowerIndex; i <= _upperIndex; i++) {
        ...
        redSum += colorPopulation * [SCPaletteColorUtils quantizedRed:color];
        greenSum += colorPopulation * [SCPaletteColorUtils quantizedGreen:color];
        blueSum += colorPopulation * [SCPaletteColorUtils quantizedBlue:color];
    }
    ...
    redMean = [SCPaletteColorUtils modifyWordWidthWithValue:redMean currentWidth:QUANTIZE_WORD_WIDTH targetWidth:8];
    greenMean = [SCPaletteColorUtils modifyWordWidthWithValue:greenMean currentWidth:QUANTIZE_WORD_WIDTH targetWidth:8];
    blueMean = [SCPaletteColorUtils modifyWordWidthWithValue:blueMean currentWidth:QUANTIZE_WORD_WIDTH targetWidth:8];

    NSInteger rgb888Color = redMean << 2 * 8 | greenMean << 8 | blueMean;
    
    SCPaletteSwatch *swatch = [[SCPaletteSwatch alloc]initWithColorInt:rgb888Color population:totalPopulation];
...
- (void)fitBox{
    ...
    for (NSInteger i = _lowerIndex; i <= _upperIndex; i++) {
        ...
        NSInteger r = [SCPaletteColorUtils quantizedRed:color];
        NSInteger g =  [SCPaletteColorUtils quantizedGreen:color];
        NSInteger b =  [SCPaletteColorUtils quantizedBlue:color];
...
@property (nonatomic,strong) SCPriorityBoxArray *priorityArray;
...
- (void)startToAnalyzeImage{
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_HIGH, 0), ^{
        ...
        for (int pixelIndex = 0 ; pixelIndex < self.pixelCount; pixelIndex++){
            ...
            red = [SCPaletteColorUtils modifyWordWidthWithValue:red currentWidth:8 targetWidth:QUANTIZE_WORD_WIDTH];
            green = [SCPaletteColorUtils modifyWordWidthWithValue:green currentWidth:8 targetWidth:QUANTIZE_WORD_WIDTH];
            blue = [SCPaletteColorUtils modifyWordWidthWithValue:blue currentWidth:8 targetWidth:QUANTIZE_WORD_WIDTH];
        ...
        if (distinctColorCount <= kMaxColorNum){
            ...
            for (NSInteger i = 0;i < distinctColorCount ; i++){
                NSInteger color = [self.distinctColors[i] integerValue];
                NSInteger population = hist[color];
                
                NSInteger red = [SCPaletteColorUtils quantizedRed:color];
                NSInteger green = [SCPaletteColorUtils quantizedGreen:color];
                NSInteger blue = [SCPaletteColorUtils quantizedBlue:color];
                
                red = [SCPaletteColorUtils modifyWordWidthWithValue:red currentWidth:QUANTIZE_WORD_WIDTH targetWidth:8];
                green = [SCPaletteColorUtils modifyWordWidthWithValue:green currentWidth:QUANTIZE_WORD_WIDTH targetWidth:8];
                blue = [SCPaletteColorUtils modifyWordWidthWithValue:blue currentWidth:QUANTIZE_WORD_WIDTH targetWidth:8];
                
                color = red << 2 * 8 | green << 8 | blue;
                
                SCPaletteSwatch *swatch = [[SCPaletteSwatch alloc]initWithColorInt:color population:population];
                [swatchs addObject:swatch];
            }
            
            self.swatchArray = [swatchs copy];
        }else{
            self.priorityArray = [[SCPriorityBoxArray alloc]init];
...
- (void)splitBoxes:(SCPriorityBoxArray*)queue{
...
- (NSArray*)generateAverageColors:(SCPriorityBoxArray*)array{
    NSMutableArray *swatchs = [[NSMutableArray alloc]init];
    NSMutableArray *vboxArray = [array getVBoxArray];
    for (VBox *vbox in vboxArray){
        SCPaletteSwatch *swatch = [vbox getAverageColor];
        if (swatch){
            [swatchs addObject:swatch];
        }
    }
...
- (void)initTargetsWithMode:(PaletteTargetMode)mode{
    ...
    if (mode < VIBRANT_PALETTE || mode > ALL_MODE_PALETTE || mode == ALL_MODE_PALETTE){
        ...
        SCPaletteTarget *vibrantTarget = [[SCPaletteTarget alloc]initWithTargetMode:VIBRANT_PALETTE];
        [targets addObject:vibrantTarget];
        
        SCPaletteTarget *mutedTarget = [[SCPaletteTarget alloc]initWithTargetMode:MUTED_PALETTE];
        [targets addObject:mutedTarget];
        
        SCPaletteTarget *lightVibrantTarget = [[SCPaletteTarget alloc]initWithTargetMode:LIGHT_VIBRANT_PALETTE];
        [targets addObject:lightVibrantTarget];
        
        SCPaletteTarget *lightMutedTarget = [[SCPaletteTarget alloc]initWithTargetMode:LIGHT_MUTED_PALETTE];
        [targets addObject:lightMutedTarget];

        SCPaletteTarget *darkVibrantTarget = [[SCPaletteTarget alloc]initWithTargetMode:DARK_VIBRANT_PALETTE];
        [targets addObject:darkVibrantTarget];

        SCPaletteTarget *darkMutedTarget = [[SCPaletteTarget alloc]initWithTargetMode:DARK_MUTED_PALETTE];
...
- (void)findMaxPopulation{
    ...
    for (NSInteger i = 0; i <_swatchArray.count ; i++){
        SCPaletteSwatch *swatch = [_swatchArray objectAtIndex:i];
...
- (void)getSwatchForTarget{
    ...
    SCPaletteColorModel *recommendColorModel;
    for (NSInteger i = 0;i<_targetArray.count;i++){
        SCPaletteTarget *target = [_targetArray objectAtIndex:i];
        [target normalizeWeights];
        SCPaletteSwatch *swatch = [self getMaxScoredSwatchForTarget:target];
        if (swatch){
            SCPaletteColorModel *colorModel = [[SCPaletteColorModel alloc]init];
...
- (SCPaletteSwatch*)getMaxScoredSwatchForTarget:(SCPaletteTarget*)target{
    CGFloat maxScore = 0;
    SCPaletteSwatch *maxScoreSwatch = nil;
    for (NSInteger i = 0 ; i<_swatchArray.count; i++){
        SCPaletteSwatch *swatch = [_swatchArray objectAtIndex:i];
...
- (BOOL)shouldBeScoredForTarget:(SCPaletteSwatch*)swatch target:(SCPaletteTarget*)target{
...
- (CGFloat)generateScoreForTarget:(SCPaletteTarget*)target swatch:(SCPaletteSwatch*)swatch{
    NSArray *hsl = [swatch getHsl];
