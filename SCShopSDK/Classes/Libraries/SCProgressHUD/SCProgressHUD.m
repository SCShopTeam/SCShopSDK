...
#error SCProgressHUD is ARC only. Either turn on ARC for the project or use -fobjc-arc flag
#endif

#import "SCProgressHUD.h"
#import "SCIndefiniteAnimatedView.h"
#import "SCProgressAnimatedView.h"
#import "SCRadialGradientLayer.h"

NSString * const SCProgressHUDDidReceiveTouchEventNotification = @"SCProgressHUDDidReceiveTouchEventNotification";
NSString * const SCProgressHUDDidTouchDownInsideNotification = @"SCProgressHUDDidTouchDownInsideNotification";
NSString * const SCProgressHUDWillDisappearNotification = @"SCProgressHUDWillDisappearNotification";
NSString * const SCProgressHUDDidDisappearNotification = @"SCProgressHUDDidDisappearNotification";
NSString * const SCProgressHUDWillAppearNotification = @"SCProgressHUDWillAppearNotification";
NSString * const SCProgressHUDDidAppearNotification = @"SCProgressHUDDidAppearNotification";

NSString * const SCProgressHUDStatusUserInfoKey = @"SCProgressHUDStatusUserInfoKey";

static const CGFloat SCProgressHUDParallaxDepthPoints = 10.0f;
static const CGFloat SCProgressHUDUndefinedProgress = -1;
static const CGFloat SCProgressHUDDefaultAnimationDuration = 0.15f;
static const CGFloat SCProgressHUDVerticalSpacing = 12.0f;
static const CGFloat SCProgressHUDHorizontalSpacing = 12.0f;
static const CGFloat SCProgressHUDLabelSpacing = 8.0f;
...
@property (nonatomic, strong) SCRadialGradientLayer *backgroundRadialGradientLayer;
...
@property (nonatomic, strong) SCProgressAnimatedView *ringView;
@property (nonatomic, strong) SCProgressAnimatedView *backgroundRingView;
...
+ (SCProgressHUD*)sharedView {
    static dispatch_once_t once;
    
    static SCProgressHUD *sharedView;
...
+ (void)setDefaultStyle:(SCProgressHUDStyle)style {
    [self sharedView].defaultStyle = style;
}

+ (void)setDefaultMaskType:(SCProgressHUDMaskType)maskType {
    [self sharedView].defaultMaskType = maskType;
}

+ (void)setDefaultAnimationType:(SCProgressHUDAnimationType)type {
...
+ (void)setForegroundColor:(UIColor*)color {
    ...
    [self setDefaultStyle:SCProgressHUDStyleCustom];
}
...
+ (void)showWithMaskType:(SCProgressHUDMaskType)maskType {
    SCProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
...
+ (void)showWithStatus:(NSString*)status {
    [self showProgress:SCProgressHUDUndefinedProgress status:status];
}

+ (void)showWithStatus:(NSString*)status maskType:(SCProgressHUDMaskType)maskType {
    SCProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
    [self setDefaultMaskType:maskType];
...
+ (void)showProgress:(float)progress maskType:(SCProgressHUDMaskType)maskType {
    SCProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
    [self setDefaultMaskType:maskType];
...
+ (void)showProgress:(float)progress status:(NSString*)status maskType:(SCProgressHUDMaskType)maskType {
    SCProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
    [self setDefaultMaskType:maskType];
...
+ (void)showInfoWithStatus:(NSString*)status maskType:(SCProgressHUDMaskType)maskType {
    SCProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
    [self setDefaultMaskType:maskType];
...
+ (void)showSuccessWithStatus:(NSString*)status maskType:(SCProgressHUDMaskType)maskType {
    SCProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
    [self setDefaultMaskType:maskType];
...
+ (void)showErrorWithStatus:(NSString*)status maskType:(SCProgressHUDMaskType)maskType {
    SCProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
    [self setDefaultMaskType:maskType];
...
+ (void)showImage:(UIImage*)image status:(NSString*)status maskType:(SCProgressHUDMaskType)maskType {
    SCProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
    [self setDefaultMaskType:maskType];
...
+ (void)dismissWithCompletion:(SCProgressHUDDismissCompletion)completion {
...
+ (void)dismissWithDelay:(NSTimeInterval)delay completion:(SCProgressHUDDismissCompletion)completion {
...
- (instancetype)initWithFrame:(CGRect)frame {
    if((self = [super initWithFrame:frame])) {
        ...
        // Set default values
        _defaultMaskType = SCProgressHUDMaskTypeNone;
        _defaultStyle = SCProgressHUDStyleLight;
        _defaultAnimationType = SCProgressHUDAnimationTypeFlat;
        ...
        _infoImage    = SCIMAGE(@"schud_info");
        _successImage = SCIMAGE(@"schud_success");
        _errorImage   = SCIMAGE(@"schud_error");
        ...
        _fadeInAnimationDuration = SCProgressHUDDefaultAnimationDuration;
        _fadeOutAnimationDuration = SCProgressHUDDefaultAnimationDuration;
        ...
        self.accessibilityIdentifier = @"SCProgressHUD";
...
- (void)updateHUDFrame {
    ...
    hudWidth = SCProgressHUDHorizontalSpacing + MAX(labelWidth, contentWidth) + SCProgressHUDHorizontalSpacing;
    
    // |-spacing-content-(labelSpacing-label-)spacing-|
    hudHeight = SCProgressHUDVerticalSpacing + labelHeight + contentHeight + SCProgressHUDVerticalSpacing;
    if(self.statusLabel.text && (imageUsed || progressUsed)){
        // Add spacing if both content and label are used
        hudHeight += SCProgressHUDLabelSpacing;
    }
    ...
    CGFloat centerY;
    if(self.statusLabel.text) {
        CGFloat yOffset = MAX(SCProgressHUDVerticalSpacing, (self.minimumSize.height - contentHeight - SCProgressHUDLabelSpacing - labelHeight) / 2.0f);
    ...
    if(self.progress != SCProgressHUDUndefinedProgress) {
    ...
    // Label
    if(imageUsed || progressUsed) {
        centerY = CGRectGetMaxY(imageUsed ? self.imageView.frame : self.indefiniteAnimatedView.frame) + SCProgressHUDLabelSpacing + labelHeight / 2.0f;
    } else {
        centerY = CGRectGetMidY(self.hudView.bounds);
    }
...
- (void)updateMotionEffectForXMotionEffectType:(UIInterpolatingMotionEffectType)xMotionEffectType yMotionEffectType:(UIInterpolatingMotionEffectType)yMotionEffectType {
    ...
    effectX.minimumRelativeValue = @(-SCProgressHUDParallaxDepthPoints);
    effectX.maximumRelativeValue = @(SCProgressHUDParallaxDepthPoints);
    
    UIInterpolatingMotionEffect *effectY = [[UIInterpolatingMotionEffect alloc] initWithKeyPath:@"center.y" type:yMotionEffectType];
    effectY.minimumRelativeValue = @(-SCProgressHUDParallaxDepthPoints);
    effectY.maximumRelativeValue = @(SCProgressHUDParallaxDepthPoints);
...
#else
            // If SCProgressHUD is used inside an app extension add it to the given view
...
- (NSDictionary*)notificationUserInfo {
    return (self.statusLabel.text ? @{SCProgressHUDStatusUserInfoKey : self.statusLabel.text} : nil);
}
...
- (void)controlViewDidReceiveTouchEvent:(id)sender forEvent:(UIEvent*)event {
    [[NSNotificationCenter defaultCenter] postNotificationName:SCProgressHUDDidReceiveTouchEventNotification
                                                        object:self
    ...
    if(CGRectContainsPoint(self.hudView.frame, touchLocation)) {
        [[NSNotificationCenter defaultCenter] postNotificationName:SCProgressHUDDidTouchDownInsideNotification
                                                            object:self
                                                          userInfo:[self notificationUserInfo]];
    }
}
...
- (void)showProgress:(float)progress status:(NSString*)status {
    __weak SCProgressHUD *weakSelf = self;
    [[NSOperationQueue mainQueue] addOperationWithBlock:^{
        __strong SCProgressHUD *strongSelf = weakSelf;
        if(strongSelf){
...
- (void)showImage:(UIImage*)image status:(NSString*)status duration:(NSTimeInterval)duration {
    ...
    [[NSOperationQueue mainQueue] addOperationWithBlock:^{
        ...
        if(strongSelf){
            ...
            strongSelf.progress = SCProgressHUDUndefinedProgress;
            [strongSelf cancelRingLayerAnimation];
            [strongSelf cancelIndefiniteAnimatedViewAnimation];
...
- (void)fadeIn:(id)data {
    ...
    if(self.defaultMaskType != SCProgressHUDMaskTypeNone) {
    ...
    if(self.backgroundView.alpha != 1.0f) {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:SCProgressHUDWillAppearNotification
                                                            object:self
                                                          userInfo:[self notificationUserInfo]];
        ...
        __block void (^completionBlock)(void) = ^{
            ...
            if(self.backgroundView.alpha == 1.0f){
                ...
                // Post notification to inform user
                [[NSNotificationCenter defaultCenter] postNotificationName:SCProgressHUDDidAppearNotification
                                                                    object:self
                                                                  userInfo:[self notificationUserInfo]];
...
- (void)dismissWithDelay:(NSTimeInterval)delay completion:(SCProgressHUDDismissCompletion)completion {
    ...
    [[NSOperationQueue mainQueue] addOperationWithBlock:^{
        ...
        if(strongSelf){
            ...
            // Post notification to inform user
            [[NSNotificationCenter defaultCenter] postNotificationName:SCProgressHUDWillDisappearNotification
                                                                object:nil
            ...
            __block void (^completionBlock)(void) = ^{
                ...
                if(self.backgroundView.alpha == 0.0f){
                    ...
                    // Post notification to inform user
                    [[NSNotificationCenter defaultCenter] postNotificationName:SCProgressHUDDidDisappearNotification
                                                                        object:strongSelf
                                                                      userInfo:[strongSelf notificationUserInfo]];
...
- (UIView*)indefiniteAnimatedView {
    ...
    if(self.defaultAnimationType == SCProgressHUDAnimationTypeFlat){
        // Check if spinner exists and is an object of different class
        if(_indefiniteAnimatedView && ![_indefiniteAnimatedView isKindOfClass:[SCIndefiniteAnimatedView class]]){
            [_indefiniteAnimatedView removeFromSuperview];
            _indefiniteAnimatedView = nil;
        }
        
        if(!_indefiniteAnimatedView){
            _indefiniteAnimatedView = [[SCIndefiniteAnimatedView alloc] initWithFrame:CGRectZero];
        }
        
        // Update styling
        SCIndefiniteAnimatedView *indefiniteAnimatedView = (SCIndefiniteAnimatedView*)_indefiniteAnimatedView;
...
- (SCProgressAnimatedView*)ringView {
    if(!_ringView) {
        _ringView = [[SCProgressAnimatedView alloc] initWithFrame:CGRectZero];
    }
...
- (SCProgressAnimatedView*)backgroundRingView {
    if(!_backgroundRingView) {
        _backgroundRingView = [[SCProgressAnimatedView alloc] initWithFrame:CGRectZero];
...
- (UIColor*)foregroundColorForStyle {
    if(self.defaultStyle == SCProgressHUDStyleLight) {
        return [UIColor blackColor];
    } else if(self.defaultStyle == SCProgressHUDStyleDark) {
...
-(UIView *)backgroundView {
    ...
    // Update styling
    if(self.defaultMaskType == SCProgressHUDMaskTypeGradient){
        if(!_backgroundRadialGradientLayer){
            _backgroundRadialGradientLayer = [SCRadialGradientLayer layer];
        }
        ...
        if(self.defaultMaskType == SCProgressHUDMaskTypeBlack){
            _backgroundView.backgroundColor = [UIColor colorWithWhite:0 alpha:0.4];
        } else if(self.defaultMaskType == SCProgressHUDMaskTypeCustom){
...
- (void)fadeInEffects {
    if(self.defaultStyle != SCProgressHUDStyleCustom) {
        // Add blur effect
        UIBlurEffectStyle blurEffectStyle = self.defaultStyle == SCProgressHUDStyleDark ? UIBlurEffectStyleDark : UIBlurEffectStyleLight;
...
- (void)setDefaultAnimationType:(SCProgressHUDAnimationType)animationType {
