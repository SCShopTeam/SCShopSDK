...
//
static unsigned char sc_base64EncodeLookup[65] =
...
//
static unsigned char sc_base64DecodeLookup[256] =
{
    ...
    size_t *outputLength)
{
    ...
    while (i < length)
    {
        ...
        while (i < length)
        {
            unsigned char decode = sc_base64DecodeLookup[inputBuffer[i++]];
            if (decode != xx)
            {
    ...
    size_t *outputLength)
{
    ...
    while (true)
    {
        ...
        for (; i + BINARY_UNIT_SIZE - 1 < lineEnd; i += BINARY_UNIT_SIZE)
        {
            ...
            //
            outputBuffer[j++] = (char)sc_base64EncodeLookup[(inputBuffer[i] & 0xFC) >> 2];
            outputBuffer[j++] = (char)sc_base64EncodeLookup[((inputBuffer[i] & 0x03) << 4)
                | ((inputBuffer[i + 1] & 0xF0) >> 4)];
            outputBuffer[j++] = (char)sc_base64EncodeLookup[((inputBuffer[i + 1] & 0x0F) << 2)
                | ((inputBuffer[i + 2] & 0xC0) >> 6)];
            outputBuffer[j++] = (char)sc_base64EncodeLookup[inputBuffer[i + 2] & 0x3F];
        }
...
+ (NSData *)dataFromBase64String:(NSString *)aString
{
    ...
    size_t outputLength;
    void *outputBuffer = sc_NewBase64Decode([data bytes], [data length], &outputLength);
...
- (NSString *)base64EncodedString
{
    ...
    char *outputBuffer =
        sc_NewBase64Encode([self bytes], [self length], false, &outputLength);
